...
| `scripts/m0_read.py` | M0, as plan version 31 reads it: the departure from one-for-one, and M-H1 to M-H4 each against it |
| `scripts/pcap_read.py` | the TCP segments of a tcpdump capture, Ethernet or Linux cooked, read without a library |
...
| `cloud/azure/stop_when_idle.sh` | calls `stop_self.sh` once the work named in `WORK` has been absent for two looks in a row, five minutes apart. For a pair whose work is not on its list: the queue's own holding job gives up after twelve hours, and the Arm pair's A3 had sixteen left when this was written, so a list armed to stop would have ended and deallocated the pair four hours before its campaign did. The pattern comes in the environment rather than on the command line, so the waiter never counts itself as work. Started from outside the checkout, as `~/stop_when_idle.sh`, on a pair that is mid-work, because an untracked file inside it is what made `git pull` abort quietly on the second x86 pair for a day |
