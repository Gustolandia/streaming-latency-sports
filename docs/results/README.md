...
| T1–T4 | what eleven benchmarking tools report | **round 1 of 4 done on the first x86 pair, all eleven tools, 23–25 Sep; rounds 2–4 running from 25 Sep.** In round 1 all eleven measure the path. Ten time against one clock, and `rdkafka_performance`, the one arranged across two, moved with its clock. Judged under freeze 21 as written. Five faults found writing it up are in the table below, and none changes a tool's answer → [`tools/t1-t4-eleven-tools.md`](tools/t1-t4-eleven-tools.md) |
...
**All eleven tools measure the path, and ten of them time against one clock.** Round 1 of T1–T4
ran on the first x86 pair from 23 to 25 September; the plan asks for four, and rounds 2–4 run
from 25 September. Every tool's figure follows the added delay at the rate
...
| T2's clock offset was spelled `-1.812ms`, and libfaketime has no millisecond unit: it read 1.812 minutes and ignored the s, moving the clock back 108 seconds | T2 refusing to run an offset it could not confirm, on its first real run | none -- the refusal is the check working; every spelling is now fractional seconds |
| and the confirmation could not see a millisecond offset at all: starting faketime and date costs about two milliseconds, more than the offset, so a correct spelling measured as -0.33 ms where 1.812 was asked | the same refusal, on a machine with nothing wrong with it | none -- the cost is the same at any offset, so it now cancels in a difference: 1.812 ms recovers as 1.806 |
| nats-latency's patterns were written from its documented report and anchored at the start of the line; the tool puts a clock on every line | T4 stopping because the tool had printed no latency at all, with 107us, 254us and 1.631ms in the file beside the message | none -- the raw output is on disk, so those runs are read again rather than repeated |
| wrk2 is not installed on the Arm pair and ten staircase folders were measured against it anyway; every one holds `exec of "wrk" failed` and an empty reading | reading what the tools had actually produced, rather than that the jobs had exited 0 | about half an hour of one pair; the folders are quarantined and wrk2 moves to the x86 pair, where it did build |
| rabbitmq-perftest was recorded as installed because a jar existed; Maven leaves a plain perf-test jar with no Main-Class beside the runnable one, and that was the first in the glob | every run of it printing "no main manifest attribute" | twelve folders, quarantined; the build now takes the first jar whose manifest has a Main-Class and no stage starts a tool that installed.json does not record as present |
| the second x86 driver recorded commit 8f31c3e on every run for a day while four tracked files were not that commit's: they had been copied onto it one at a time rather than pulled, and every `git pull` since aborted on them, quietly | asking why that pair was still four commits behind the other two, and reading `git status` on it rather than the commit it reported | none to the science: all four are real versions from this repository, newer than the commit claimed, and the two that decide things are the gate and brake freeze 16 asks for. The record was wrong, and the collector now names any file that differs from its commit |
| T2's verdict for a tool that takes both timestamps itself cannot be decided: shifting one clock moves both readings equally, so no value comes out below zero | the judge saying so from the data on valkey-benchmark's first complete T2, rather than being told | none yet -- but T2's question needs restating for self-timing tools, and that is a result about the method rather than a fault in it |
| the load correction stopped the first stress-ng with a plain TERM and then waited on it; stress-ng does not stop for that here, so the wait sat until the process reached its own `--timeout 3600s` | four gaps of exactly 3762 s in a campaign log whose median gap is 166 s, and every one of them a run that took the correction branch | 62.7 minutes per affected run instead of 2.8, about one run in nine. The first x86 pair lost four runs of its A2 that way and the second spent four of the twelve hours its job is given, reaching 35 of 60 runs before the queue took the job back. The runs themselves stand: each ran under the corrected duty, with an hour of the designed duty before it rather than seven seconds, and `load_correction.json` says which. The kill is now -9, as the cleanup at the end of the same function has always been |
| rdkafka_performance's consumer was never attached to a partition -- its usage says `-p <num>` "defaults to random" and the invocation gave none -- so it read none of its producer's messages | the shakedown plan version 23 asks for, running the same invocation with -p 0 added and nothing else changed: it read every message sent | a whole staircase of ten steps that measured nothing, reported as nothing rather than as zero. Two more things came out of the same trials: the start offset defaults to the beginning, not the end as the note claimed, so a run read the previous run's messages and reported their age as latency; and unpaced, what the tool reports is its own one-second batching rather than a trip. Nothing of this tool has been measured yet |
| `cloud/azure/session.sh` was run with AZ_PROFILE=arm to rebuild the Arm pair's session. That variable only names the files the script writes; the pair it acts on comes from HOSTS_ENV, which defaults to the first x86 pair. So it restarted that pair's broker containers and tore down its receiver namespace, mid-campaign | the script printing 10.1.1.11 where the Arm pair's receiver is 10.1.1.31 | the first x86 pair's A2 Kafka campaign stopped itself at 44 of 60 runs, on its own rule, after three runs failed the check that the receiver's namespace holds its address -- the check doing exactly what it is for. The queue put the job back on the end of the list for one more go and moved to the next, so the campaign is re-run whole and nothing is analysed from the stopped one. About three hours of one pair. Mine, not the machines' |
| k6 made no request in any of its fifteen runs: every stage runs it through `sudo`, which clears the environment its URL and rate came through, so it looped 4,895,826 times in 60 s on "invalid URL" and sent no byte; the reader fell back to k6's iteration time when the request metric was absent and reported the 9 microseconds a failure took as a latency | fitting each tool's T1 reading against the delay it added: every other tool rose with it, and k6's did not move at all | fifteen runs that measured nothing and were counted as fifteen readings. Set aside and run again with its settings as flags; the reader now reads the request and nothing else (freeze 21, D25-3) |
| `tools.sh brokers` was run on the driver though its header says the broker: it set the driver's servers up, asked 127.0.0.1 whether they answered, and certified a machine the tools never talk to, while the broker kept RabbitMQ's loopback-only user and had no NATS | a check that asked what each tool had actually reported, then reading what two of them printed: ACCESS_REFUSED and no servers available | thirty runs of two tools that measured nothing, and a staircase of ten more once one was rebuilt. Set aside and run again; `brokers` now does the setup on the broker and checks from the driver, the way the tools are answered, and every stage checks first (D25-4) |
...
| the tools block ran one round where D4-2 asks for four, and `tools.sh` wrote every stage into folders named only for the tool and the step, so a second round would have written over the first. Its write-up then called the block complete | checking what each pair still had to do against the plan's table of campaigns, after being asked whether all three pairs were working | none to what was measured; a night of one pair idle. Each round now has a folder of its own, no stage writes over runs already there, and rounds 2 to 4 run from 25 September |
