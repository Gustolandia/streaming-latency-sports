...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 12)}}
...
{\large Version 12, frozen on 20 September 2026. It fixes what each tool is predicted to do before any tool campaign runs, from the documentation and source audit \gl{T5} the plan asks for, and records the one prediction that audit refuted. Version 11, of the same day, settles one thing version 10 left open: a session that runs at a reduced core count measures its own baseline there as well as its own calibration. Version 10, of the same day, amends version 9 in nothing and corrects how version 9 was applied. The rounds rule is to simulate a campaign at the spread this pair's spread pilot measured \emph{for that backend}, and version 4 wrote it that way. The spread was computed over both backends together instead, and that pooled figure set the rounds for A1, A4 and A5's first simulation. What that cost is recorded below, campaign by campaign. No measured run prompted this and none has been analysed. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D12-1} mark what changed since version 11, and grey tags such as \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the eight tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
Version 12 fixes the tool predictions before any tool runs. The plan says a tool marked ``read first'' is classified before any run and that the classification is itself a prediction; seven tools carried a question mark, and the audit has now read all seven. Their classes are fixed here, with the version each was read at, so that what T1 to T4 measure can be held against something written down first. The audit also refuted one classification the plan had already made --- RabbitMQ PerfTest was predicted to keep negatives and does not; it takes their size, so a clock offset is reported as delay --- and that refutation is recorded as a failed prediction rather than quietly corrected.\chg{D12-1}\\[3pt]
...
The tools are read and their predictions fixed. No tool campaign has run, so everything here is
written before the runs it will be judged against. On the law's side nothing changes: A3 is
running under version 9, and A1, A4 and A7 have finished unanalysed.
...
P3a & \textbf{Load leaves the cliff where it is.}\chg{D9-1} At the 3~ms slice,\chgold{D4-4} the halfway point moves by less than 0.25~ms between 50\% and 88\% load. & The whole 95\% interval of the movement lies inside $\pm$0.25~ms \gl{two one-sided tests, on the signed difference}. A point estimate inside the band is not enough: a scattered estimate can land inside it by luck, and this is a claim that something did not happen. \\
