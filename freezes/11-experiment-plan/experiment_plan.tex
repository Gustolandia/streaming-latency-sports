...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 15)}}
...
{\large Version 15, frozen on 21 September 2026. It changes the tools block and nothing in the law. Before any tool was run, T1 and T2 were put through made-up tools built out of the two things that make a tool behave as its class does: how coarsely its clock reads, and whether it reads that clock once or twice. T1 works as frozen and separates the class that chops from every other on the first question it asks. T2 does not work as frozen. At the 0.5~ms offset this plan fixes, not one value of 3,000 goes below zero; at 2~ms a tool reading whole milliseconds still meets none, because truncation toward zero puts everything between minus one step and plus one step at zero, so its rule for negatives never runs. Five of the ten tools read such a clock, and they are the ones the block most wants to describe. T2's offsets are therefore taken from the trip they act on, what each behaviour would have given is worked out through the tool's own step, and the verdict names what survives rather than what is nearest. No tool campaign has run. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D15-1} mark what changed since version 14, and grey tags such as \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the eleven tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 15 changes the tools block and no prediction of the law. T1 and T2 were run against made-up tools before any real one, to find out what they could answer. T1 can answer: it reports the chopping class at 0.9~ms where every other class reports 0.1~ms. T2, as frozen, cannot. Its smaller offset makes no negative at all on a 3~ms trip, and its larger one makes none that a whole-millisecond clock can hold.\chg{D15-1} The offsets are now taken from the session's own median trip, and what each behaviour would have given is worked out through the step T1 measured for that tool, because that is what the tool's rule for odd values actually sees.\chg{D15-2} The verdict names what survives being ruled out rather than what is nearest: on a tool whose clock is coarser than the gap between two behaviours the nearest is decided by the rounding, and a run leaving two standing has not answered.\chg{D15-3} The identification is read where sends are spread evenly; sends paced in step with the tick are T-P2's evidence and not an identification.\chg{D15-4} Each figure a tool prints now carries the step its own printing can express, because two of these tools print an exact average beside percentiles chopped to whole milliseconds, and one step for the tool would lose the thing T-P1 and T-P2 predict.\chg{D15-5}\\[3pt]
...
Each runnable tool is one campaign on one machine pair, with our own program recording the same messages.\chgold{D4-1} The ten that run are Vegeta, hey, k6 and wrk2 against a fixed page this block serves; valkey-benchmark and memtier\_benchmark against Valkey; Kafka's EndToEndLatency and ProducerPerformance and librdkafka's rdkafka\_performance against Kafka; RabbitMQ PerfTest against RabbitMQ; and the NATS CLI against a NATS server. Every server is installed on the machine and not in a container, because a container adds a network namespace and firewall rules between tool and broker and this block measures tenths of a millisecond.\chg{D15-6} For each tool, at least two experiments, and three where it applies:

\textbf{T1. Delay staircase} \gl{receiver-only, as in the fix above}. Add 0, 0.1, 0.2, 0.3, 0.5, 0.7, 0.9, 1.1, 1.5 and 2.0~ms, in random order. Record the tool's average, p50 and p99, and our reference at each step. Each figure is recorded with the step its own printing can express, not one step for the tool: two of these tools print an exact average beside percentiles chopped to whole milliseconds, and that gap is what T-P1 and T-P2 predict.\chg{D15-5} The smallest step the tool reports at all is its answer here, and it is what T2 is then given.\chg{D15-2} \emph{Questions:} does the tool see a 0.1~ms step? Does it round, chop or keep? Does its average track the truth?

\textbf{T2. Forced negatives.} Make the receiver's clock read early by the session's own median trip plus 1~ms, and plus 3~ms\chg{D15-1} \gl{libfaketime for Java, C and Python tools, checked on each tool first; a second machine with a deliberately offset clock for Go tools, restored afterwards}. The offsets are taken from the trip because a fixed one cannot reach a tool with a coarse clock: truncating toward zero, as Java and C do, puts every value between minus one step and plus one step at zero, so the offset has to clear that step before the tool meets a negative at all.\chg{D15-2} \emph{Questions:} does it average over the negatives, drop them, replace them with zero, crash, or count them? Does its message count still match what was sent? \emph{How it is judged:} what each of the four behaviours would have produced --- its average, its smallest value and its count --- is worked out from our own record of the same messages, put through the step T1 measured for that tool, and each behaviour is ruled out by a figure it cannot account for.\chg{D15-2} One left standing is the answer; two is a run that did not answer, and is reported as that rather than as the nearest.\chg{D15-3} The identification is read where sends are spread evenly; sends paced in step with the tick are run and reported as T-P2's evidence.\chg{D15-4} An offset that leaves a tool nothing to report at all is recorded as too large for it.\chg{D15-4}
