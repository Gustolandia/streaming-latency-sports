...
{\large Version 16, frozen on 21 September 2026. It corrects one brake and no prediction. A8 takes its ``got it'' note two ways, and taking it inline puts one message in flight where a calibration runs with sixty-four: two different producers, on purpose, because that is the treatment A8 exists to measure. The brake that asks whether the added delay moved the got-it was holding those runs against a calibration that took the note the other way, so it was measuring the treatment. On the Arm pair it stopped A8 on its first Python run --- 0.167~ms against a 0.145~ms brake --- while the Java runs, whose two medians happen to lie 0.06~ms apart, went through untouched. A brake that stops one client and not the other, for a difference neither was asked about, is not a check on the instrument. It now compares a run only against a baseline that took the note the same way, and a campaign says before it runs how many of its runs it can judge. The brake is not loosened: a run measured the calibration's way, moving that far, still stops. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D16-1} mark what changed since version 15, and grey tags such as \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twelve tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
