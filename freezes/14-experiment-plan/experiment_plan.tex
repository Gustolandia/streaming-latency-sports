...
{\large Version 18, frozen on 21 September 2026. It widens one brake, and it does so after that brake stopped a campaign, which is the order of events that ought to make a reader suspicious. The argument is not that the check failed. Version 17 holds a run against its own campaign's earlier runs of the same setup and allows three times \emph{their} scatter, and it lets the brake judge as soon as two such runs exist --- so the first braked run of every setup is measured against a standard deviation taken from two points, which has one degree of freedom and is not an estimate of spread. A2's first session shows what that costs: its ten setups gave within-setup scatters from 0.053 to 0.227~ms around a pooled 0.133, and the campaign was stopped by the setup that happened to give 0.053 --- a run 0.275~ms from its setup's centre, which three times the pooled figure would have admitted with room to spare. The scatter belongs to the sitting, not to one setup. It is now pooled across the campaign's setups, the centre is unchanged, and the brake waits until that pool has five degrees of freedom. Nothing is cut: a run the brake cannot yet judge counts, and what the brake would have used travels beside it. It corrects one brake and no prediction, for the second time in a day and in the opposite direction: version 16 asked which runs the brake may judge, and this asks what it may judge them against. The brake held every run against the session's calibration, which is a different sitting. A campaign sits some hours from its calibration and its ``got it'' drifts in that time, by an amount that has nothing to do with the delay added --- and the allowance it is given is a quarter of that delay. So the allowance is smallest exactly where the drift has nowhere to go. Measured: A3's Kafka campaign on the first pair drifted 0.022~ms from its calibration over 216 judged comparisons, A8's on the Arm pair drifted 0.124 over 14, and their noise floors were 0.170 and 0.199 --- so it is the drift that differs, six-fold, and not the floor. The shift does not grow with the delay (slope $+$0.004~ms per ms across a ladder from 0.88 to 3.46~ms) while the allowance does. A run is now held against its own campaign's earlier runs of the same setup, which is like for like and has no drift in it; the distance from the calibration is still computed and recorded, and no longer stops anything. P5(c) is judged where this plan has always judged it, in the session's own calibration, which varies the delay with everything else fixed.  No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D18-1} mark what changed since version 17, and grey tags such as \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the fourteen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
