...
{\large Version 19, frozen on 22 September 2026. It changes no prediction and no campaign is re-judged. What it corrects is a program: the judge of P8 said in its own documentation that it tested Java's cliff \emph{and the priority effect}, and tested Java's cliff and the ratio between the two clients. This plan has asked since version 8 that go-first cut Java's plateau at least five times, and no program computed it. The clause is now computed from the campaign's own runs --- A8 runs every setup ordinary and go-first, so nothing further had to be measured --- gated with the other two, and written beside the verdict. It was found by reading the rule the judge printed against the rule this plan states, while judging A8's first finished campaign, and that campaign has been judged again under all three clauses: the verdict does not move, and the cut the judge now measures, 469.7, is the figure computed by hand from the same runs. An instrument corrected on a campaign that has already answered can be checked against an answer it cannot have influenced. The second change is scheduling and not a rule: the ten tool campaigns run on the first x86 pair rather than five on each. What this plan requires is that each tool runs entirely on one pair, and the five-and-five split was how the load was balanced when it was written; the second x86 pair now carries A5's two remaining sessions and all of A2, about 46 hours, against the tools' 47. Recorded here too, because it is the kind of thing a reader should not have to take on trust: counting the Arm pair's finished campaigns against A4's design, before giving that pair new work, found that A4 had run three of its four campaigns since 18 September and that the fourth had never been started. It is running now, at the rounds its three siblings ran. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D19-1} mark what changed since version 18, and grey tags such as \chgold{D18-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the fifteen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
