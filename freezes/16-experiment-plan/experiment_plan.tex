...
{\large Version 20, frozen on 22 September 2026. It widens two brakes and changes no prediction. Neither widening touches a measurement: every run is recorded exactly as before, with its got-it median, its distance from its own setup's centre and its distance from the session's calibration beside it, and P5(c) is judged from the calibration on the same numbers as ever. What changes is only when a campaign is allowed to stop itself, and both changes are made because a brake stopped work that was not wrong. The first: the calibration's precision bound follows the machine's scheduling tick. It was a flat 0.30~ms, set on the stock kernel whose tick is 1~ms, and A2 boots two kernels where the tick is 4 and 10. A calibration cannot be known to a fraction of the quantum its trips are quantised by, and the measurements say so plainly: on the second x86 pair Kafka came in at 0.410 at HZ=250 and 0.387 at HZ=100, against 0.128 to 0.237 for Redis on the same boots, and four rounds did not mend it because it is not imprecision to be averaged away. The bound is now the larger of 0.30~ms and 0.15 of the tick, and the bound each session used is written beside its gate. The second: the got-it brake's floor goes from 0.10~ms to 0.25. 0.10 is P5(c)'s own equivalence margin, which is the right size for the question P5(c) asks and the wrong size for this one, which is whether the instrument broke mid-campaign. A8's x86 campaign stopped at 86 of 144 runs on a move of 0.134~ms where its own pooled scatter allowed 0.132 --- an eight-hour campaign ended on an overshoot of two microseconds, after four earlier stops. Three times a very tight scatter is a very tight allowance, and A8's within-setup scatter is 0.012 to 0.036~ms; the floor is what stops the allowance collapsing onto the noise, and at 0.10 it sat below the drift sound campaigns show. Also recorded, because it is the kind of thing a reader should not have to take on trust: a campaign was until today refused unless \emph{every} backend's calibration passed, whichever backend it ran, which threw away three sessions and about seven and a half hours of one pair; and the tools block's install step had never fetched or built a single tool, which nothing had noticed because T1 and T2 were checked against made-up tools. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D20-1} mark what changed since version 19, and grey tags such as \chgold{D19-1}, \chgold{D18-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the sixteen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
