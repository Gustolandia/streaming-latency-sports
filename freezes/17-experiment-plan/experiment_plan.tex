...
{\large Version 21, frozen on 22 September 2026. It moves half of one campaign to the other x86 pair and changes no prediction. Every prediction in this plan is judged within one machine pair and one backend, and nothing is pooled across either; so A2's three-kernel comparison for Kafka is whole on whichever pair runs it, and the counterbalancing this plan asks for --- each kernel on two days in a balanced order --- is kept for each backend separately. The reason is the shape of the work left, which is the reason version 19 moved the tool campaigns: the first x86 pair finishes the ten tools today and the second is carrying about forty-five hours on its own. Splitting A2 by backend has the last measurement land about nineteen hours earlier, and leaves the session the second pair has already run at HZ=1000 with Kafka as a replicate of that one cell on a second machine. Also here, because a reader should not have to take it on trust: until today a prediction was judged on every run that had a trip and a negative rate, whatever the brake had said about it, although the brake itself has always refused to compare a run against neighbours that did not count; five runs of 3,179 carry another verdict, and re-judging without them reproduces A3's two answers to four decimal places and moves A8's ratio from 3.12 to 3.25 with the same verdict. And A4's P9 was read with the block's four campaigns in one folder, so every run carried that folder's name, the anchor correction --- which lines campaigns up by grouping them --- found one campaign and moved everything by zero, and the answer said each campaign had been moved by its own offset. Read as this plan asks, the free slope is 0.830 on Kafka and 1.153 on Redis, both inside the band the law demands, and what fails is the fitted summary of the same curves; P9 is not confirmed either way. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D21-1} mark what changed since version 20, and grey tags such as \chgold{D20-1}, \chgold{D19-1}, \chgold{D18-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the seventeen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
