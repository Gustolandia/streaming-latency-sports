...
{\large Version 22, frozen on 22 September 2026. It reads one gate across the staircase instead of step by step, and changes no prediction. The gate asks whether the added delay is reaching the ``got it'' note, which would spoil the note every later run is held against. Leakage of a fraction of the delay is that fraction at every step, so it is a slope; the test was a quarter of the delay step by step, which is not the same thing at all. A note that drifts by a fixed amount fails wherever that amount is more than a quarter of the delay added --- every step below four times the drift, and no step above it. The second x86 pair showed both halves of that in one calibration: Redis shifted +0.342~ms at 1~ms added and the session was ended, while Kafka shifted $-$0.315 at 4~ms and $-$0.282 at 8, both further from zero than their own 90\% intervals reach and both larger than the one that failed, and passed. Regressed on the delay, Redis's four shifts give a slope of $-$0.010 and Kafka's $-$0.038: nothing was leaking in either, and an hour of calibrating was spent on a threshold that has no relation to what it tests. The test is now that slope, with a shift larger than the whole delay added still gross at any one step on its own. Everything else about the note is unchanged: every step is still tested for equivalence at 90\%, a step not shown equivalent still does not fail the gate, and every number is still written out. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D22-1} mark what changed since version 21, and grey tags such as \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D18-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the eighteen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
