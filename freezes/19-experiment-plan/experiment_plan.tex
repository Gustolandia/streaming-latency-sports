...
{\large Version 23, frozen on 22 September 2026. It adds an addendum at the end (Section~\ref{sec:addendum}) and one run to each T2 session, and changes no prediction. The tools block ran for the first time on 22 September, and running it showed something about T2 that reading it had not: T2 moves the clock a tool reads and asks what the tool does with the values that come out below zero, and that question has an answer only where the tool's subtraction spans two clocks. Ten of the eleven tools are invoked as a single process that writes both timestamps itself, so the moved clock moves both, the difference is untouched, and no value below zero can arise. The eleventh, rdkafka\_performance, is the only one arranged with the producer on the real clock and the consumer on the moved one --- and its consumer read none of the producer's messages, so it measured nothing at all. Neither is a defect in the tools or in T2: it is the thing this block exists to report, because the artifact this paper is about is a property of one-way latency measured across two clocks. A benchmark that reports a round trip against its own clock cannot exhibit it, and cannot measure what a streaming system cares about either. So T2 now says which of the two a tool is and shows it, from a no-offset run taken in the same session and the offset measured at the clock; and rdkafka\_performance's arrangement is established by a shakedown before anything of it is measured. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D23-1} mark what changed since version 22, and grey tags such as \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D18-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the nineteen tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
