...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 16)}}
...
{\large Version 24, frozen on 24 September 2026. It gives the Arm pair one more campaign and changes no prediction. A3 asks what load does --- whether it leaves the cliff where it is \gl{P3a} and whether it raises the plateau \gl{P3b} --- and both have only ever been judged on the first x86 pair. A4 already asks A1's question of a second processor family, and no campaign asks this one. So A3's Kafka campaign runs on the Arm pair as well, at \textbf{24 rounds}, the number P3a's simulation gives at that pair's own spread and levels. Its Redis half is not run: that backend's plateau is too shallow there to carry the question, as it is on the x86 pair, and the Arm pair's own spread pilot says so in its own numbers. P3b is run and reported as underpowered before a run of it exists, because the rule cannot reach the power it asks for at any number of rounds this plan would spend. Nothing is pooled across pairs: the x86 answer stands as it is and the Arm answer will be its own. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D24-1} mark what changed since version 23, and grey tags such as \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 24 gives the Arm pair A3's Kafka campaign and changes no prediction. \chg{D24-1} A3's Kafka campaign runs on the Arm pair at 24 rounds, from P3a's simulation at that pair's own spread and levels: confirmed in 88\% of campaigns under the law and in 0\% of campaigns where it is false. Load has been asked of one processor family only, and the second part of this paper is whether the effect appears on machines it was never seen on. \chg{D24-2} A3's Redis campaign is not run there: the Arm pair's spread pilot reads a plateau of 0.0115 and a floor its own reader marks as below resolution, which is the same shallow curve that puts P3a and P4 out of reach on Redis on the x86 pair. \chg{D24-3} P3b is judged on the same runs and is recorded as underpowered before any of them exist: 76\% at the ceiling of 40 rounds where the rule asks 80\%, so a non-confirmation there is a limit of this instrument and not evidence against the prediction.\\[3pt]
Version 23 adds an addendum and one run to each T2 session, and changes no prediction. \chg{D23-1} A T2 session runs three times rather than two: once at no offset and once at each of its two offsets, with the offset actually achieved measured at the clock and written beside every run. Where the clock moved and the tool's figures did not, the tool times against one clock and no value below zero can arise in it --- a verdict rather than the undecided this produced before, and the one the artifact's own premise predicts. \chg{D23-2} rdkafka\_performance, the only tool arranged across two clocks, has that arrangement established by a shakedown before anything of it is measured: its consumer read none of its producer's messages over a whole staircase, and a tool that measures nothing must not be reported as a tool that measured zero. The addendum at the end sets out both, with how each of the eleven tools keeps time.\\[3pt]
Version 22 reads one gate across the staircase instead of step by step and changes no prediction. \chg{D22-1} A gross departure of the ``got it'' note is the added delay reaching it, and that is a shift which grows with the delay, so it is read as the slope of the shift against the delay and fails the gate above a quarter. Step by step, the same quarter could not tell leakage from drift: a constant drift fails at every step below four times its size and passes above, so the second x86 pair's Redis ended a sound session on +0.342~ms at 1~ms added while its Kafka passed on $-$0.315 at 4~ms and $-$0.282 at 8. Their slopes are $-$0.010 and $-$0.038. A shift larger than the whole delay added is still gross at any one step on its own, and the slope is written out beside the gate.\\[3pt]
Version 21 moves A2's Kafka half to the first x86 pair and changes no prediction. \chg{D21-1} The six Kafka sessions and the Kafka bridge run on the first x86 pair; the second keeps Redis, A5's two remaining sessions and the Redis bridge. Nothing is pooled across pairs or backends in any judgement here, so each backend's comparison of the three kernels stays whole on the machine that ran it, with each kernel on two days as \chgold{D4-7} asks. The first pair has been idle-bound behind the tools and the second was carrying about forty-five hours alone; this has the last measurement land about nineteen hours earlier, and the Kafka session the second pair already ran becomes a replicate of that cell on a second machine. \chg{D21-2} A prediction is judged on the runs the integrity rule passed, and the answer names the ones it left out. The reader that feeds every judge kept anything with a trip and a negative rate; a stopped run is one whose own instrument was in doubt and a repeated one was run again, and \texttt{run\_integrity.py} has always refused to compare a run against such neighbours. Five runs of 3,179 are affected: A3's P3a and P3b reproduce to four decimals without them and A8's Python-to-Java ratio moves from 3.12 to 3.25, not confirmed either way.\\[3pt]
Version 20 widens two brakes and changes no prediction. Both are widened because they stopped work that was not wrong, and neither changes what is written down about any run. \chg{D20-1} The calibration's precision bound follows the tick: the larger of 0.30~ms and 0.15 of it, where it was a flat 0.30 set on a 1~ms tick. A2 boots kernels whose tick is 4 and 10~ms, and a calibration cannot be known to a fraction of the quantum its trips are quantised by. Kafka on the second x86 pair came in at 0.410 at HZ=250 and 0.387 at HZ=100 against Redis's 0.128 to 0.237 on the same boots, and four rounds did not move it. The bound each session used is written beside its gate, and the check is no longer called \texttt{known\_within\_0\_3\_ms}, which would now be a name that lies. \chg{D20-2} The got-it brake's floor goes from 0.10~ms to 0.25. A8's x86 campaign stopped at 86 of 144 on a move of 0.134~ms where its own pooled scatter allowed 0.132; the floor is what stops that allowance collapsing onto a very tight scatter, and at 0.10 it sat below the drift sound campaigns show --- 0.124~ms on the Arm pair, 0.134 here. It is still a brake: a campaign that moves 0.4~ms at a 0.5~ms delay still stops. \chg{D20-3} A campaign is gated on the calibration it is placed from rather than on every one in the file; until today a Redis campaign was refused for the state of Kafka's fit, which cost three sessions and about seven and a half hours of one pair.\\[3pt]
Version 19 corrects a program and no prediction. P8 says that Java shows the same cliff \emph{and priority effect} as Python, and that Python's rate is at most 1.5 times Java's. Three clauses; the judge tested two. It printed the rule it was applying, and the rule it printed named the cliff and the ratio --- while the function's own documentation said it tested the priority effect. Nothing about that is visible in a verdict: a judge that omits a clause a campaign passes returns exactly what a complete one would. \chg{D19-1} Go-first's cut of Java's plateau is now computed at the trips both clients ran, gated at the five times this plan has asked for since version 8, and recorded beside the verdict. A8's Arm campaign has been judged again under all three clauses and returns the same answer --- not confirmed, on the ratio, at 4.33 where 1.5 is allowed --- with a cut of 469.7. \chg{D19-2} The ten tool campaigns run on the first x86 pair rather than five on each, which changes no rule: each tool still runs entirely on one pair, and that is what this plan requires of them. This is the third program in two days found to report on less than it claimed, after the two of 21 September, and the three share a shape: each was caught by asking a judge what it actually did rather than reading what it said.\\[3pt]
Version 18 widens one brake, after that brake stopped a campaign. The order of events deserves suspicion, so here is the argument that does not depend on it: version 17 allows a run three times the scatter of its own setup's earlier runs, and lets the brake judge as soon as two of them exist. A standard deviation from two points has one degree of freedom. It is not an estimate of spread, and version 17 guarantees that the first braked run of every setup is measured against one. A2's first session, ten setups over 28 counted runs, gave within-setup scatters of 0.053, 0.055, 0.082, 0.082, 0.116, 0.127, 0.177, 0.185, 0.224 and 0.227~ms --- one common quantity seen through two or three points each, pooling to 0.133 --- and it was the 0.053 that stopped the campaign, on a run 0.275~ms from its setup's centre. Three times the pooled figure is 0.398. \chg{D18-1} The scatter is now pooled over the campaign's own setups, from each run's distance to its own setup's centre; the centre a run is held against does not change. \chg{D18-2} The brake waits until that pool has five degrees of freedom, and until then a run is recorded rather than braked. \chg{D18-3} The pooled scatter and its degrees of freedom are written beside every run, so the allowance can be recomputed by anyone who doubts it. The brake is not switched off: a run a whole millisecond from its setup's centre still stops this campaign. And nothing is cut --- a run the brake cannot yet judge still counts, with its got-it beside it.\\[3pt]
Version 17 corrects one brake and no prediction, and it is the same brake version 16 corrected the day before. Version 16 settled \emph{which} runs it may judge. This settles \emph{what it judges them against}. The brake held each run's ``got it'' median against the session's calibration --- a different sitting, hours earlier --- and allowed it a quarter of the delay that run added. Those are two different quantities: what separates a campaign from its calibration is drift, which does not grow with the delay, and the allowance does. A3's Kafka campaign drifted 0.022~ms from its calibration over 216 judged comparisons and A8's drifted 0.124 over 14, on noise floors of 0.170 and 0.199 --- near-equal floors, a six-fold difference in drift --- and the brake bit at exactly one rung: the shortest delay of the client that needs the least delay, where its allowance falls to 0.221~ms. \chg{D17-1} A run is now held against its own campaign's earlier runs of the same setup, which shares its client, its note's place, its slice and its delay, so no drift enters the comparison; until a setup has two such runs its got-it is recorded and not braked. \chg{D17-2} The allowance keeps its shape and its quarter-share, now measured against those runs. \chg{D17-3} The distance from the session's calibration is still computed and written beside every run, and no longer stops anything; P5(c) stays where this plan has always put it, in the session's own calibration, which varies the added delay with everything else fixed and is the experiment P5(c) describes. The brake is not loosened: checked against A8's own runs, a campaign that broke mid-run would still stop.\\[3pt]
Version 16 corrects one brake and no prediction. P5(c) asks whether the added delay moved the ``got it'' time, and the brake that enforces it compared every run against the session's calibration. A8 takes that note two ways, and taking it inline forces the producer to one message in flight where the calibration ran with sixty-four, so the comparison asked what A8's own treatment did rather than what the delay did.  \chg{D16-1} A run whose note is taken a way the calibration never took it is now recorded rather than braked: its got-it median and where the note was taken travel with it, and P5(c) is asked of those runs by comparing the ones that share a note's place across the delays they ran at.\chg{D16-2} And a campaign counts, from its own queue before anything runs, how many of its runs the brake can judge and how many it cannot, because on the Arm pair that was found on the third run and three hours in.\chg{D16-3}\\[3pt]
Version 15 changes the tools block and no prediction of the law. T1 and T2 were run against made-up tools before any real one, to find out what they could answer. T1 can answer: it reports the chopping class at 0.9~ms where every other class reports 0.1~ms. T2, as frozen, cannot. Its smaller offset makes no negative at all on a 3~ms trip, and its larger one makes none that a whole-millisecond clock can hold.\chgold{D15-1} The offsets are now taken from the session's own median trip, and what each behaviour would have given is worked out through the step T1 measured for that tool, because that is what the tool's rule for odd values actually sees.\chgold{D15-2} The verdict names what survives being ruled out rather than what is nearest: on a tool whose clock is coarser than the gap between two behaviours the nearest is decided by the rounding, and a run leaving two standing has not answered.\chgold{D15-3} The identification is read where sends are spread evenly; sends paced in step with the tick are T-P2's evidence and not an identification.\chgold{D15-4} Each figure a tool prints now carries the step its own printing can express, because two of these tools print an exact average beside percentiles chopped to whole milliseconds, and one step for the tool would lose the thing T-P1 and T-P2 predict.\chgold{D15-5}\\[3pt]
Version 14 is the first written after a campaign was run and read. A3's Kafka campaign confirms that load raises the plateau, and leaves P3a unresolved -- not for want of rounds but because at 50\% load the negative rate is 0.15\%, and no cliff can be located in a curve that shallow. Neither more rounds nor more messages narrow it, and both were checked against that campaign's own runs.\chgold{D14-3} P3a is now judged where the ordinary plateau is at least 2\%, the guard P4 has carried since version 4; simulated at A3's measured levels that lifts it from 9\% power to 98\%, with false confirmations at 1\%.\chgold{D14-2} A load below the bar is reported as out of reach, as an unreachable slice already is.\chgold{D14-3} The rounds rule is simulated at the plateau and floor the pilot measures rather than at 0.30 and 0.01.\chgold{D14-1} The world those simulations draw on now counts negatives out of a run's messages and moves the cliff between runs, because one spread multiplied onto a rate can represent neither.\chgold{D14-4}\\[3pt]
...
\textbf{A3. Load.}\chgold{D4-4} Run at 50\%, 75\% and 88\% load, at the 3~ms slice and 6 trips, in two campaigns, one per backend. Each campaign calibrates at all three loads. Its rounds come from P3a.\chg{D9-1} \emph{Questions:} does load leave the cliff where it is \gl{P3a}, and does it raise the plateau \gl{P3b}? The Kafka campaign runs on the Arm pair too, at 24 rounds from that pair's own simulation\chg{D24-1}; the Redis campaign does not, because that pair's plateau is as shallow as the x86 pair's\chg{D24-2}. P3b is underpowered on the Arm pair at any number of rounds this plan would spend, and is reported as such before its runs exist\chg{D24-3}.
...
\begin{itemize}
  ...
  \item The session's delay calibration passed P5(b), and the run shows no gross ``got it'' departure from its own campaign's earlier runs of the same setup.\chg{D17-1} Its distance from the session's calibration is recorded beside it and brakes nothing \gl{P5(c) is judged in the calibration itself}.\chg{D17-3}
...
\textbf{Which pair runs what.} With one x86 pair, every x86 campaign runs on it and S0-4 is not needed. With two x86 pairs, the first runs S0-1 to S0-3, A1, A3, A7, A8's x86 campaign and five tool campaigns; the second runs S0-4, A5, all of A2, and the other five tool campaigns. The Arm pair runs A4, A8's Arm campaign and A3's Kafka campaign\chg{D24-1} --- not A3's Redis campaign, which that pair's plateau cannot carry\chg{D24-2}.
...
Stops the campaign & The instrument is in doubt: a message arrived before it was sent \gl{P5(a)}, or the run's ``got it'' median moved from the median of its own campaign's earlier runs of the same setup by more than the allowance \gl{a quarter of the delay it added, three times the scatter pooled over this campaign's setups, or 0.10~ms, whichever is largest}.\chgold{D17-1}\chg{D18-1} A run at a setup with fewer than two earlier counted runs is recorded rather than braked, and its distance from the session's calibration is recorded for every run and brakes none.\chg{D17-3} & The run is recorded as failed, and nothing more runs on that pair until the cause is found and recorded with the campaign. If the cause is in the code every pair runs, the other pairs stop too. \\
\bottomrule
\end{longtable}}
...
\section[Addendum: how each tool keeps time]{Addendum: how each tool keeps time, and the runs that decide it\protect\chg{D23-1}}\label{sec:addendum}
