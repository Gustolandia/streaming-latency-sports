...
{\large Version 25, frozen on 24 September 2026. It records every correction found on 24 September, when a check of the running campaigns was turned on the instrument itself, and it changes no prediction. Three of the eleven tools had measured nothing while reporting figures or errors that looked like readings: k6 made no request at all and reported the time its failures took, to the microsecond; nats-latency and rabbitmq-perftest met servers that could not answer them, because the setup meant for the broker had been run on the driver and certified there. Those runs are set aside and run again. T2's judge is given what it lacked, an allowance for each tool's own run-to-run noise, measured from the ten runs of that tool's T1 staircase; the rule is fixed here, before any reading is judged under it (Section~\ref{sec:addendum-t2}). And the Arm pair's A3 is read at the loads it actually ran at. No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D25-1} mark what changed since version 24, and grey tags such as \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-one tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\textbf{A3. Load.}\chgold{D4-4} Run at 50\%, 75\% and 88\% load, at the 3~ms slice and 6 trips, in two campaigns, one per backend. Each campaign calibrates at all three loads. Its rounds come from P3a.\chgold{D9-1} \emph{Questions:} does load leave the cliff where it is \gl{P3a}, and does it raise the plateau \gl{P3b}? The Kafka campaign runs on the Arm pair too, at 24 rounds from that pair's own simulation\chgold{D24-1}; the Redis campaign does not, because that pair's plateau is as shallow as the x86 pair's\chgold{D24-2}. P3b is underpowered on the Arm pair at any number of rounds this plan would spend, and is reported as such before its runs exist\chgold{D24-3}.
...
\textbf{Which pair runs what.} With one x86 pair, every x86 campaign runs on it and S0-4 is not needed. With two x86 pairs, the first runs S0-1 to S0-3, A1, A3, A7, A8's x86 campaign and five tool campaigns; the second runs S0-4, A5, all of A2, and the other five tool campaigns. The Arm pair runs A4, A8's Arm campaign and A3's Kafka campaign\chgold{D24-1} --- not A3's Redis campaign, which that pair's plateau cannot carry\chgold{D24-2}.
...
\section[Addendum: T2's judgement, and the corrections of 24 September]{Addendum: T2's judgement, and the corrections of 24 September\protect\chg{D25-2}}\label{sec:addendum-t2}
...
\begin{enumerate}
  ...
  \item A hypothesis \textbf{survives} when every judged figure's change lies within the
        allowance of what it predicts. A count short of what was sent still rules out the
        behaviours that could not have given it, as before.
