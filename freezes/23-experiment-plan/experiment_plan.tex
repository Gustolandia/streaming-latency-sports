...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 27)}}
...
{\large Version 27, frozen on 25 September 2026. It adds the runs that could still improve what the testbed has measured before it is taken down, and records what the check of all the data on 25 September found in the programs that judge; it changes no prediction. A2's Redis session on our HZ=1000 build, which had one day of its two, runs the second (D27-1); each backend gets a second bridge day on the stock kernel (D27-2); A1 gains a 3.75~ms slice, so that Kafka has the four slices P1 asks for (D27-3); and a third x86 A3 campaign runs at the two loads P3a and P3b are judged at (D27-4). No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D27-1} mark what changed since version 26, and grey tags such as \chgold{D26-1}, \chgold{D25-1}, \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-three tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 27 adds the runs that could still improve the results before the testbed is taken down, and records what the check of all the data found in the judging programs; it changes no prediction. \chg{D27-1} A2's Redis session on our HZ=1000 build runs its second day again, whole. \chg{D27-2} Each backend gets a second bridge day on the stock kernel. \chg{D27-3} A1 gains a slice of 3.75~ms and one Kafka campaign at 3 and 3.75~ms. \chg{D27-4} A third x86 A3 Kafka campaign runs at 75 and 88\% load only, on the second x86 pair. \chg{D27-5} Five places where the judging programs depart from this plan's text are recorded, to be corrected in version 28 with every verdict they touch judged again. \chg{D27-6} Two studies of the mechanism, a trace of the got-it helper's own waits and M0, are frozen in version 28 before they run.}
...
\textbf{Which pair runs what.} With one x86 pair, every x86 campaign runs on it and S0-4 is not needed. With two x86 pairs, the first runs S0-1 to S0-3, A1, A3, A7, A8's x86 campaign and five tool campaigns; the second runs S0-4, A5, all of A2, and the other five tool campaigns. The Arm pair runs A4, A8's Arm campaign and A3's Kafka campaign\chgold{D24-1} --- not A3's Redis campaign, which that pair's plateau cannot carry\chgold{D24-2}. Version 27 gives the first x86 pair A1's Kafka campaign at 3 and 3.75~ms and the Kafka bridge's second day, and the second x86 pair the second day of A2's Redis session at HZ=1000, the Redis bridge's second day and the third x86 A3 campaign.\chg{D27-1}
