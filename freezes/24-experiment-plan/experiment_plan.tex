...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 28)}}
...
{\large Version 28, frozen on 25 September 2026. It adds A9, a study of the mechanism on all three pairs: the waits of the one thread that stamps each acknowledgement, recorded event by event and read against each negative reading, with how it is read fixed here before its runs (D28-1). It changes no prediction. M0, and the corrections version 27 records, come in version 29 (D28-2). No run this plan covers starts before it is public (Section~\ref{sec:freeze}).}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D28-1} mark what changed since version 27, and grey tags such as \chgold{D27-1}, \chgold{D26-1}, \chgold{D25-1}, \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-four tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 28 adds A9 and changes no prediction. \chg{D28-1} A9 records, on all three pairs, the waits of the one thread that stamps each acknowledgement, event by event, and reads each negative reading against them; the clients name the threads that stamp. How it is read is fixed before its runs. \chg{D28-2} M0, with the recording it never had, and the corrections of D27-5 come in version 29, frozen before M0 runs.}
...
\textbf{Which pair runs what.} With one x86 pair, every x86 campaign runs on it and S0-4 is not needed. With two x86 pairs, the first runs S0-1 to S0-3, A1, A3, A7, A8's x86 campaign and five tool campaigns; the second runs S0-4, A5, all of A2, and the other five tool campaigns. The Arm pair runs A4, A8's Arm campaign and A3's Kafka campaign\chgold{D24-1} --- not A3's Redis campaign, which that pair's plateau cannot carry\chgold{D24-2}. Version 27 gives the first x86 pair A1's Kafka campaign at 3 and 3.75~ms and the Kafka bridge's second day, and the second x86 pair the second day of A2's Redis session at HZ=1000, the Redis bridge's second day and the third x86 A3 campaign.\chgold{D27-1} Version 28 gives each of the three pairs A9's campaign.\chg{D28-1}
