...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 30)}}
...
{\large Version 30, frozen on 25 September 2026. It makes the five corrections version 27 recorded in the programs that judge (D30-1 to D30-5), fixes how A9 is read where this plan left the choice to the reading, and adds a second reading of A9-2 beside it before any A9 run (D30-6). M0's reading comes in version 31, before M0's runs are read (D30-7). It changes no prediction and adds no run.}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D30-1} mark what changed since version 29, and grey tags such as \chgold{D29-1}, \chgold{D28-1}, \chgold{D27-1}, \chgold{D26-1}, \chgold{D25-1}, \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-six tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 30 corrects the programs that judge and changes no prediction. \chg{D30-1} P3a and P3b compare the lowest and the highest load whose plateau is at least 2\%, as D14-2 says; every A3 verdict until now compared 50\% load with 88\%. \chg{D30-2} P7 is held to P1's band at each core count, and to nothing else. \chg{D30-3} A campaign that cannot reach what a rule reads is reported out of reach, not as a test that went against the law. \chg{D30-4} A block of several campaigns is resampled campaign by campaign. \chg{D30-5} P6 has a program, and the recordings in half of A1's and A3's runs are read against it. \chg{D30-6} How A9 is read is fixed where the plan left it open, and a second reading of A9-2, message by message, stands beside the one version 28 froze. \chg{D30-7} M0's reading comes in version 31. Every verdict D30-1 to D30-4 touch is judged again, with the reading as it was beside it.}
...
\textbf{A6. Predict from a recording, with nothing fitted.} In a random half of A1 and A3 runs, record the helper's waits in 0.1~ms steps, and predict the negative rate from that recording. Tracing slows things slightly, so traced and untraced runs are compared too. \emph{As read, version 30:}\chg{D30-5} a run's rate is predicted as the share of its recorded waits longer than $T - g$, its median trip less its median ``got it'' delay \gl{E-A9's prediction; the recording holds every \texttt{python3} thread, with no clock}, and judged by P6's rule on each pair and backend, by \texttt{helper\_waits.py p6}.
