...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 31)}}
...
{\large Version 31, frozen on 25 September 2026. It fixes how M0 is read, before any of M0's runs is read (D31-1 to D31-3). It changes no prediction, no test of M-H1 to M-H4 and no run.}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D31-1} mark what changed since version 30, and grey tags such as \chgold{D30-1}, \chgold{D29-1}, \chgold{D28-1}, \chgold{D27-1}, \chgold{D26-1}, \chgold{D25-1}, \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-seven tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 31 fixes how M0 is read and changes no prediction. \chg{D31-1} The departure is signed and read on the recorded runs, and an explanation accounts for half of it only by pushing the same way. \chg{D31-2} M-H2's replay runs each recorded run's messages through the receive loop's own rules and costs, never through its outcomes. \chg{D31-3} Both clients switch Nagle's algorithm off, so only the send window can hold a request for an acknowledgement; M-H4 is run as written, and not for Kafka.}
