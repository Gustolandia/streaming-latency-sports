...
\hypersetup{pdftitle={Experiment plan: turning the two findings into laws (version 32)}}
...
{\large Version 32, frozen on 26 September 2026. Every campaign from here on runs with the got-it brake recording rather than stopping (D32-1), and every campaign that stopped before its runs were all done is finished, on its own kernel and from its own queue (D32-2). Two faults of the kit that cut runs are corrected (D32-3), and every run's machine and settings are registered beside every fault met (D32-4). It changes no prediction.}\\[2pt]
{\color{soft}\small Grey text in brackets is a plain-word explanation. Green tags such as \chg{D32-1} mark what changed since version 31, and grey tags such as \chgold{D31-1}, \chgold{D30-1}, \chgold{D29-1}, \chgold{D28-1}, \chgold{D27-1}, \chgold{D26-1}, \chgold{D25-1}, \chgold{D24-1}, \chgold{D23-1}, \chgold{D22-1}, \chgold{D21-1}, \chgold{D20-1}, \chgold{D19-1}, \chgold{D17-1}, \chgold{D16-1}, \chgold{D15-1}, \chgold{D14-1}, \chgold{D13-1}, \chgold{D12-1}, \chgold{D11-1}, \chgold{D10-1}, \chgold{D9-1}, \chgold{D8-1}, \chgold{D7-1}, \chgold{D6-1}, \chgold{D5-2} and \chgold{D3-5} what changed in earlier versions; the twenty-eight tables below say what each change is and why. Prices are Azure's list prices as read again on 16 September 2026, and they change.}
...
\boxit{Version 32 finishes what was started and changes no prediction. \chg{D32-1} The got-it brake records rather than stops in every campaign from here on, as it did for A5's last runs; each prediction it touches is judged with and without the runs it would have stopped. \chg{D32-2} A campaign that stopped before its runs were all done is finished from its own queue, under its own calibration, on the kernel it ran on. \chg{D32-3} The kit's two faults that cut runs are corrected. \chg{D32-4} Every run's machine, kernel and settings are registered, and every fault met is recorded.}
