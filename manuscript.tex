...
transport climbing steeply with $N$ (to 774~ms at $N{=}5$ and 9{,}732~ms at $N{=}20$) while
\kafka{} stays flat. That effect is real, but it is driven by \emph{aggregate event rate}, not
by the number of concurrent streams: the merged plan packs ten matches into every feed, so its
``$N{=}5$'' applies roughly ten times the load of five real matches. Football event data is
intrinsically low-rate---0.44 events per second per match---so expressing each tested load in
simultaneous real-time match equivalents (Table~\ref{tab:regime}) locates the boundary
honestly. Single-node \redis{} serves the rate of $\approx$1{,}200 simultaneous matches in
1.4~ms and only degrades around $\approx$6{,}400. For scale, the top five European leagues
field roughly fifty simultaneous matches on a busy weekend. \redis{}'s single-threaded
serialization is therefore a genuine architectural property that is nonetheless irrelevant at
any load a football operator will encounter.

\begin{table}[htbp]
\centering
\caption{Where the architectural difference actually appears. Loads are expressed in
simultaneous real-time match equivalents at 0.44 events/s per match.}
\label{tab:regime}
\begin{tabular}{rrrr}
\toprule
...
calibration because both backends are evaluated against the identical model. Third, concurrent
feeds replay recorded matches on a compressed clock rather than consuming genuinely live
sources; we mitigate the most damaging form of this by giving each feed a \emph{distinct}
match, which is what revealed the throughput confound discussed in
Section~\ref{sec:concurrency}. Fourth, our null result is a failure to reject rather than a
proof of equivalence: a two-one-sided-tests procedure against a pre-specified indifference
margin (for example, one video frame of staleness) would state it positively, and we regard
that as the natural next step. Finally, the study covers football event data specifically; the
...
to roughly 6{,}400 simultaneous real-time matches---two orders of magnitude beyond a busy
