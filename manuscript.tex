...
On the corrected corpus, with each concurrent feed carrying a \emph{different} real match at true per-match event rates, we find: (i)~\kafka{} and \redis{} are statistically indistinguishable at every concurrency level tested ($\approx$10\,ms median \tti{}; Holm--Bonferroni-corrected Mann--Whitney $p>0.5$ throughout), and neither exhibits a concurrency effect (Kruskal--Wallis $p=0.70$ and $0.20$); (ii)~single-node \redis{} \emph{does} serialize concurrent streams, but only far outside realistic load---football event data is intrinsically low-rate at 0.44 events per second per match, and a dedicated throughput sweep places the onset of \redis{} degradation near 2{,}100 events/s, equivalent to roughly 4{,}800 simultaneous matches, while 2{,}400 match equivalents are still served in 2.5\,ms; and (iii)~translated into decisions through an Age-of-Information win-probability integral, a goal's win probability is stale by $\approx$0.014 probability-seconds ($\approx$12\,ms) irrespective of backend or concurrency. The practical conclusion is a quantified negative result: across the entire operating envelope a football deployment will ever encounter, streaming-infrastructure choice does not measurably degrade in-play decision quality, and backends should be selected on durability and operational grounds rather than latency.
...
than copies of one merged plan, the answer is a quantified negative: it does not, at any load
football produces. \kafka{} and \redis{} are statistically indistinguishable at every
...
football, on open data with open code. The practical message is that practitioners should
choose a backend on durability and operational grounds rather than latency. The methodological
message is twofold: cross-process streaming benchmarks demand a shared clock, a non-saturating
load generator and symmetrically configured clients; and a concurrency axis that is secretly a
