...
\redis{} has no second thread and inverts on a larger share of runs (Supplement~S12.3). The
...
% Round 55 moved the denominator to Supplement S15.3, where the exhibits are set out. With
...
other's (Supplement~S12.1); Section~\ref{sec:brokers} says what that is worth.
...
$\untracedRate$ is the machine's (Supplement~S12.4).
...
$\exposureCrossover$--$\exposureCrossoverHi$~ms (Supplement~S12 bands every row and draws
the curve).
...
one factor every time. The same bootstrap rejects a fit here too (Supplement~S8). Neither
...
microseconds and holds $\tracedModeShareA\%$ of all wakeups; a second mode at
$128$--$256\,\mu$s holds $\tracedModeShareB\%$; the third, holding
...
and \tti{} is free of it (Supplement~S12.1).
...
clients differ by a payload parse (Supplement~S12.1), and no reader could have known.
...
and neither argued for (Supplement~S15.2). Nor is it unwelcome: the tool we audit
accepted a coordinated-omission report and now records the publish delay, and the
admission condition came through that revision untouched; a later request to
serialize the histograms it discards after reading their summaries would carry the
deletion into a new container unless the retained fraction travels with it
(Supplement~S22).
...
% unstated assumption, are both worked through in S29 and S12.6; the main text needs the
...
admission condition and genuine negatives survive it (Supplements~S29 and~S12.6). A better
...
brackets the observed rate ($\tracedRatios$) instead of under-predicting (Supplement~S12.4).
...
% The four correspondents thanked here by name were cut on 2026-09-10. Each had
% replied once in August and none since; the lead author's instruction is that a
% paper thanks people who are still in the conversation. What they gave is still in
% the paper -- the same-clock contrast, the kernel-tracing pointers, the
% probe-placement argument and the resolution question all shaped sections that
% remain -- and the citations they led to are unchanged. The paragraph below is not
% optional: IEEE requires disclosure of generative-AI assistance.
...
% the timer-characterisation citation stays in Section III, and Figure 1, the thread
