...
$\recoveryErrFail\%$ and $\recoveryErrFailHi\%$. Those medians differ; the populations do
not. The worst condition is $\recoveryPassMax\%$ on either side and the Hodges--Lehmann
shift over $\recoveryShiftPairs$ pairs is $\recoveryShift$ points [95\% bootstrap:
$\recoveryShiftCI$]; what opens the median gap is a concentration at zero, and with the exact
recoveries set aside no shift remains, $\recoveryNonzeroShift$ points
[$\recoveryNonzeroShiftCI$] (Supplement~S16.9). The recovery therefore holds where the check
rejects as well as where it passes, which is what Section~\ref{sec:authors} asks of it.
