...
% Alphabetical, case-insensitively, and gated by test_index_terms_are_alphabetical. Round 75
% replaced the list, and the reason is that the old one described a different paper. It was
% six IEEE Computer Society taxonomy strings, each broad enough to fit a thousand articles --
% "distributed systems", "message passing", "performance attributes" -- and between them they
% did not carry one word this paper is about. A reader searching for the failure we document
% would search for the clock, the timestamp, the middleware or the benchmark, and none of the
% four appeared. Index terms are a retrieval surface, not a subject declaration: the test they
% have to pass is whether the reader who needs this paper can find it.
% The terms below are the words the title, the abstract and the section headings already use,
% and `test_round75_findings.py` holds them to that -- including the rule that every noun in
% the title appears among them. That rule is why one term differs from the referee's suggested
% list: it proposed "performance measurement", which is covered twice over by "benchmarking"
% and "latency measurement", and its own diagnostic table lists STREAMING among the words a
% searcher would use and would not find. "Stream processing" takes the slot. Five of the seven
% remain taxonomy strings -- benchmarking, clock synchronization, scheduling, stream
% processing and, at one remove, latency measurement -- so the taxonomy is not abandoned, only
% narrowed to the branches this work actually sits on.
% Round 33's referee asked for uniform case, and with the multi-word taxonomy quotation gone
% the list is uniformly lower case; the earlier decision to capitalize turned on that one
% verbatim string and lapses with it. Note that a comment *inside* the block below is parsed
% as two index terms by test_index_terms_are_alphabetical -- which is how this note ended up
% outside it.
\begin{IEEEkeywords}
benchmarking;
...
% Round 77 (W5): Chen et al., "Statistical performance comparisons of computers", IEEE TC
% 64(5), 2015 -- bib key chen2015statcomparisons -- is prepared as the swap for
% georges2007rigorous if an editor asks for a venue citation on statistical reporting. The
% referee advised against swapping unprompted: Georges is the better-known citation and every
% current reference is load-bearing. The sentence survives the swap unchanged.
...
for a CPU. We see $\rtHighBasePct\%$ at $\rtHighLoadPct\%$ utilization on one host, with an inter-host
offset of $\interHostOffsetMs$~ms where two are used. A threshold in milliseconds of skew
does not transfer to a loaded machine. The same attribution was offered for the benchmark we audit: asked why
...
sub-second event times. We characterize the full $\corpusMatches$-match corpus and replay
\replayedMatchesWord{} of its matches, preserving each match's event order
...
Pacer jitter is $\pacerJitterLo$--$\pacerJitterHi\,\mu$s at p90 across its $\pacerJitterRuns$ runs, and the achieved rate is recorded per run against
...
($\spanNegAckPct\%$; Wilson intervals are under $0.1$ points on corpus-wide rates and are
omitted where they are that narrow, not in Table~\ref{tab:mechanism}).
...
it exceeds the whole. The two are correlated within a run (median $\spanRhoWithinMedian$ over
$\spanRhoConditions$ conditions), one scheduler causing both, which suppresses crossings
without touching the displacement: treating the two as independent within a run would
overpredict them
$\indepWithinFloored$-fold, the median of the $\indepWithinFlooredN$ conditions above
$\indepFloorPct\%$.
...
by $\invGrowth$. The rate does not approach one: the highest we measure near saturation is
$\invCeiling$, a maximum over those conditions and not a bound either, so even then most
timestamps are taken by a thread that was running, or late by less than the delivery.
...
$\rho$ returns one value for one input. Each factor's bracket is a Katz $95\%$ interval on
the ratio.}
\label{tab:mechanism}
\centering
\footnotesize
...
\midrule
\multirow{2}{*}{Priority, $75\%$} & ordinary & $\rtLowBase$ & $\rtLowBaseCI$ & $\rtLowFactor\times$ ($\rtLowZ$) \\
                                        & real-time & $\rtLowRt$ & $\rtLowRtCI$ & [$\rtLowFactorCI$] \\
\addlinespace[2pt]
\multirow{2}{*}{Priority, $88\%$} & ordinary & $\rtHighBase$ & $\rtHighBaseCI$ & $\rtHighFactor\times$ ($\rtHighZ$) \\
                                        & real-time & $\rtHighRt$ & $\rtHighRtCI$ & [$\rtHighFactorCI$] \\
\midrule
\multirow{2}{*}{Geometry, original}     & concentrated & $\GeomOrigConc$ & $\GeomOrigConcCI$ & $\GeomOrigFactor\times$ ($\GeomOrigZ$) \\
                                        & spread & $\GeomOrigSpread$ & $\GeomOrigSpreadCI$ & [$\GeomOrigFactorCI$] \\
\addlinespace[2pt]
\multirow{2}{*}{Geometry, repl.}  & concentrated & $\GeomReplConc$ & $\GeomReplConcCI$ & $\GeomReplFactor\times$ ($\GeomReplZ$) \\
                                        & spread & $\GeomReplSpread$ & $\GeomReplSpreadCI$ & [$\GeomReplFactorCI$] \\
\bottomrule
\end{tabular}
\end{table}
...
and $\rtHighFactor$. Each factor's bracket in the table is a Katz interval on the ratio,
wide because the collapsed configurations are sparse; the Wilson intervals are disjoint at
both load levels. Both residual rates land on the scale the model names
...
$\GeomOrigFactor\times$, reproduced at $\GeomReplFactor\times$ by a full replication
(Table~\ref{tab:mechanism}, bottom). Multi-server queueing
...
$\payloadRateFall\times$ [Katz 95\%: $\payloadRateFallCI$], monotonically, at a $\payloadRhoSpread$
...
and $\recoveryErrPassHi\%$ at the upper quartile; over the $\recoveryFailN$ it rejects,
$\recoveryErrFail\%$ and $\recoveryErrFailHi\%$. Those medians differ; the populations cannot
be told apart. Both worst conditions are $\recoveryPassMax\%$; the Hodges--Lehmann shift,
rejected minus accepted, is $\recoveryShift$ points [95\% bootstrap:
$\recoveryShiftCI$]; what opens the median gap is a concentration at zero, and without it no shift is
detectable, $\recoveryNonzeroShift$ points [95\% bootstrap:
$\recoveryNonzeroShiftCI$] (Supplement~S16.9). The recovery therefore holds where the check
rejects as well as where it passes, as Section~\ref{sec:authors} requires.
...
descheduled at the wrong instant therefore reads its clock about a slice late ---
$\sliceOverDeliveryLo$--$\sliceOverDeliveryHi$ times the per-condition deliveries this
corpus measures, $\condDeliveryLoMs$--$\condDeliveryHiMs$~ms --- and the tick is why the
mode is a band rather than a spike. The scheduler's slice lands on the scale of the
interval being measured, which is why the check fires on a substantial minority of events
rather than on almost none or on most. A mean over this distribution is dominated by a mode the operator never
...
one millisecond does compute to zero. Section~\ref{sec:extphase} recovers this benchmark's own
delivery from its retention and puts it at $\spreadIncommensurateTrueMs$~ms, half the
resolution: about half of what a co-located run takes computes to zero, and far more
on the rates whose phase sits on the grid. Second, the one observation which would show the
...
$\ombGridMedianCells$ report a median of $\ombGridPrintedLo$ or $\ombGridPrintedHi$~ms, and across those the benchmark
...
$\ombRetentionFold$-fold difference, and both reported a median of $\ombGridPrintedLo$~ms with nothing in
the output to distinguish them. Three uninstrumented runs print p50, p95, p99 and max all
equal to $\ombGridPrintedLo$ to three decimal places. The full ledger holds $\ombRuns$ instrumented runs,
$\ombDiscarded$ discarded samples and $\ombNegatives$ negatives, and over it the retained
fraction falls as low as $\ombRetentionMinExact\%$ --- about four samples in ninety
thousand.
The printed median is pinned at the timestamp resolution while retention moves across two
and a half orders of magnitude (Fig.~\ref{fig:deletion}). The only cells that escape are the \ombEscapeCellsWord{} whose payload is large enough that
...
information about retention: $\ombGridMedianCells$ cells report $\ombGridPrintedLo$ or
$\ombGridPrintedHi$~ms --- the two labeled ticks --- while the
...
have (Spearman $\ombRetentionRho$; Fisher 95\%: $\ombRetentionRhoCI$; $\ombRetentionRhoN$ cells) carries
...
$\spreadIncommensurateHi$ --- implying $T_{\mathrm{true}} = \spreadIncommensurateTrueMs$~ms at
$\tau = 1$~ms.
...
without counting}. Substitution is the worst of the \harnessDisposalClassesWord{} classes because it leaves
...
The admission condition survives downstream: $\forkUnchanged$ of the $\forkChecked$
readable public forks carry the expression unchanged (surveyed \forkCheckedOn{}), which
...
$\tostLevels$ levels), and on \tti{}, which is a causal chain, against a wider margin over
the same levels (Supplement~S13). So it is not a purchasing argument. The one condition that reverses
...
stable for having its variation deleted, and a second tester who picks a different send
rate measures a different sample. Every rule below instances the criterion that sees this.
...
clocks have for nearly thirty years~\cite{paxson1998calibrating,paxson2004sound}. The discipline is tested only by a
...
together (median correlation $\spanRhoWithinMedian$), and assuming independence within a run
overpredicts the negative-span rate in all
$\indepWithinConditions$ conditions, by a median factor of $\indepWithinFloored$ over the
$\indepWithinFlooredN$ with a rate above $\indepFloorPct\%$.
...
we measure at $\rtHighLoadPct\%$ utilization is $\rtHighBasePct\%$ of events
(Table~\ref{tab:mechanism} has the lower load too). Run the sign check under the load
...
failed, and one unsigned total conflates them.
...
\emph{timestamping} one. Hardware-timestamped PTP~\cite{ieee1588} would take two LAN hosts
from our $\interHostOffsetUs\,\mu$s of \texttt{chrony} agreement to sub-microsecond, and
behind a one-millisecond timestamp $\interHostOffsetUs\,\mu$s and $70$~ns produce
byte-identical records. The admission condition deletes the same samples and the retention
...
% unstated assumption, are both worked through in S29 and S16.5.
% Round 72 moved this subsection's conclusion to its FIRST sentence under the rule that a
% section opens on its claim, and round 73 found what that left behind: the paragraph then
% ended on a back-reference to the ordering three lines above it. It ends on the residue
% instead, which is the half a better clock does not carry away.
Resolution is the lesser half, and a distinct one. A microsecond timestamp retires
Equation~\ref{eq:retention}'s deletions here and moves the grid to faster paths; what it
leaves untouched is the admission condition and every genuine negative
(Supplements~S29 and~S16.5).
...
$\chronyHostBoundLo$--$\chronyHostBoundHi$~ms per host; the two worst, $\chronyWorstBound$ and
$\chronySecondWorstBound$, sum to $\chronyPairBound$~ms against a $1$~ms timestamp, so the cross-host channel stays open
...
$\harnessCrossHostSamples$ two-clock samples --- the same count as the one-clock campaign of
Section~\ref{sec:generality} because the two were matched run for run, not a repeated figure. Cross-CPU incoherence under migration: below. Utilization: identical
...
regime Lozi et al.\ document, may shift them. The workload is \replayedMatchesWord{} matches
of one sport's event feed: nothing here depends on its content beyond message size and rate,
and that independence is argued rather than assumed (Supplement~S20). The deletion arithmetic
is demonstrated on one benchmark and reproduced in one independent harness. The \harnessAuditedWord{} tools of
...
reporting rules above, for pointing us to the timer study cited in
