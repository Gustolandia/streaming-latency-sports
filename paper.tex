...
of the samples taken. We identify and experimentally characterize both, show that they
materially alter reported results, and derive checks their authors can apply: an exact
identity for the negative span, a pre-registered manipulation, and an audit of
\harnessAuditedWord{} tools.
\end{abstract}
...
delivery} of a message: the interval $D$ would report if it were measured with an arbitrarily
fine timestamp, which is what $D$ measures up to the consumer's own timestamping jitter.
...
\label{sec:tools}
Neither failure is one project's oversight: the tools are few and shared, and five of 
the \harnessAuditedWord{} we read dispose of a sample without counting it.

Nor is the admission condition confined to the tool we measured: $\forkUnchanged$
of the $\forkChecked$ public forks we could read carry the expression unchanged, surveyed
\forkCheckedOn{}.

Deletion is not the only way coarse resolution wins. We audited \harnessAuditedWord{} tools at source
...
Section~\ref{sec:tools} are readings of source rather than measured deployments. The
