...
Section~\ref{sec:external} does not bind. Not all do. Of the \litComparisonsWord{} 2026
broker comparisons we placed against that resolution, \litInsideRegimeWord{} report figures
at or below it, and one names a percentile for a single broker and none for the other
two~\cite{indexdev2026brokers}, so its ranking cannot be placed against the resolution at
all.
...
of the residual delay in that state, and take the consumer's timestamp to lie in the core. Write $T_{\mathrm{true}}$ for the
...
$10$~ms delivery, $\exposureErrHundred\%$ at $100$~ms and $\exposureErrOne\%$ at $1$~ms;
below $\exposureCrossover$~ms the displacement exceeds the delivery it is a correction to,
and sub-millisecond medians are routine in broker benchmarking. $A$ is not a constant:
across our conditions it spans $\exposureLagLo$--$\exposureLagHi\,\mu$s, putting the
$10$~ms error at $\exposureErrTen$--$\exposureErrTenHi\%$ and the crossover at
% S24, not S25: the exposure table and its curve sit at the end of S24, in the block that
% collects the exhibits moved out of the main text. The pointer had said S25 since the v4
% renumbering, and round 54's exhibit-verb rule is what caught it.
$\exposureCrossover$--$\exposureCrossoverHi$~ms (Supplement~S24 bands every row and draws
the curve).
...
(Supplement~S24). The two brokers' clients timestamp differently, and over
...
Log--log least squares over the four payload levels of Section~\ref{sec:twostate} returns a
slope of $\tailSlope$ ($\tailSlopeCI$); four points do not earn an equation here, and an
earlier version read the kernel trace as confirming it. We withdraw the claim, because
estimating the traced index properly is what refuted it: two estimators disagree sixfold,
and a bootstrap goodness-of-fit rejects the fitted power law ($p \tracedGofP$ over
$\tracedGofBoot$ replicates), both in Supplement~S15.
...
the standard evaluation environment --- hides it (Supplement~S25).
...
and neither argued for (Supplement~S52.2).
