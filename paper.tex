...
\redis{} on $\spanRedisNegAckPct\%$ of its $\spanRedisEvents$.
...
$\spanRatioConditions$ conditions whose interquartile range runs
$\understateIQRLo$--$\understateIQRHi\times$ and reaches $\understateWorst\times$, so half
of them are worse than the headline. It is a median of per-condition ratios, which is not what
differencing two published medians gives (Supplement~S24). The two brokers' clients timestamp differently, and over
...
failed --- the $32$~KB arm pins at $\flipPinThirtyTwo\%$ retention, not on the predicted
