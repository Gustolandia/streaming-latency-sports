...
per-run sign check rejected $\auditRejected$ of our $\auditRuns$ runs; referenced to the
send timestamp the same events never invert on one clock by construction, so the cause is
...
embedded-mode configurations, $\ombGridMedianCells$ reported a median that was an exact
multiple of one millisecond, computed from $\ombGridRetentionMin$--$\ombGridRetentionMax\%$
of the samples taken. We contribute an exact identity for the
negative span, a law for the retained fraction confirmed by a pre-registered manipulation,
and an audit of \harnessAuditedWord{} tools. The remedy is to randomize the send instant
and publish the retained fraction.
\end{abstract}
...
a consumer reads them back. The four timestamps and the three spans they bracket are
defined in Section~\ref{sec:sysmodel}; the transport proxy, the span this paper is about,
subtracts a producer-process timestamp from a consumer-process one. Both processes run on
...
\caption{\textbf{A late timestamp, not an early record.} How a positive latency is measured
...
acknowledgment's timestamping delay exceeds the delivery. The path drawn is \kafka{}'s;
\redis{} has no second thread and inverts on a larger share of runs (Supplement~S43.3). The
...
and the paths they compare now complete in well under a millisecond. This paper reports two
ways such a benchmark misreports on those paths without any sign of it in its output. It
then says what a benchmark must publish for a reader to tell.
...
clock, and each of which waits for that core on its own account. On a busy machine either
wait can exceed the delivery being measured. When the two waits differ by more than the
...
fixed quantity, and they act differently. Sub-millisecond broker paths are where each
...
binds is the scheduler's: the last mode of the run-queue stall distribution sits at
$\baseSliceMs$~ms (Section~\ref{sec:tail}), three times the timestamp resolution and nothing
...
alone.
\begin{enumerate}
\item \textbf{A law for what a positivity filter discards} (Section~\ref{sec:external}).
The fraction of samples a millisecond-resolution benchmark retains is fixed by the ratio of
its send interval to its timestamp resolution. It is never shown to the operator. A
pre-registered manipulation of message size moves a configuration onto and off the values
the law predicts.
...
% happened is the supplement's spine now (its S35), and an introduction that narrates its own
...
The paper is organized as follows. Section~\ref{sec:sysmodel} defines the system and the
measurement model, Section~\ref{sec:related} places the work against prior measurement
studies, Section~\ref{sec:method} gives the experimental setup and the validation rule,
Sections~\ref{sec:external} and~\ref{sec:mechanism} present the two failures with their
evidence, and Section~\ref{sec:discussion} draws the reporting rules.
...
Section~\ref{sec:proxy}. The primary end-to-end measure is \textbf{Time-to-Insight}
...
\label{eq:tti}
\end{equation}
which telescopes to $t_{\mathrm{recv}} - t_{\mathrm{sched}}$ (Supplement~S45 maps every
metric of the corpus onto the span it covers). A \emph{span} is any difference of two of
these timestamps, and a \emph{negative span} is one whose difference is below zero. The
term is Sharma et al.'s~\cite{sharma2026causality} (a concurrent preprint), kept for the
observable while declining the causal reading they give it. It is not RFC~1242's negative
latency, a cut-through forwarding artifact~\cite{rfc1242}. \emph{Retention} is the
...
Since Equation~\ref{eq:model} holds exactly per event, with $D$ and $A$ both non-negative,
\begin{equation}
...
$\diseaseEvents$ events of our corpus, and the build fails if they part. Every timestamp is
...
is check what its tool discards. Most published comparisons report a median above the
tool's one-millisecond timestamp resolution, where the deletion law of
Section~\ref{sec:external} does not bind. Not all do, and a 2026 comparison that calls two of
three brokers sub-millisecond without naming a percentile cannot be placed against that
resolution at all.

The tools are few and shared, so a defect in one is not one project's problem. The audit of
Section~\ref{sec:generality} reads \harnessAuditedWord{} tools at source and finds
...
two-process measurement: a law relating the negative-span rate to the delivery being
measured (Equation~\ref{eq:twostate}, Section~\ref{sec:mixture}), and manipulations that
separate scheduling from its rival explanations. The audit half has its own ancestor.
...
a span's timestamps are called harmful and default to off, and nobody had measured how much
such a correction moves.
...
estimator; the benchmark discards that population. Our contribution is not the law ---
...
Chandrasekar and Kramberger~\cite{chandrasekar2026bias}, in a concurrent preprint, apply
...
running the brokers under containers, produced our withdrawn first result and appears only
as an audit outcome.
...
manipulated in Section~\ref{sec:mechanism}.
...
establish it from data instead (Section~\ref{sec:mechanism}; Supplement~S42). The Time Stamp Counter (TSC) is a
...
its content beyond message size and rate (Supplement~S11). The send schedule is measured rather than assumed.
...
other's (Supplement~S43.1); Section~\ref{sec:brokers} says what that is worth.
...
is a threshold, not a discovery. The supplement sweeps it from $0$ to $20\%$, and no
condition behind our first result is usable anywhere in that range, so the withdrawal does
not turn on where the line was drawn.
...
declines to publish from a run whose measurement demonstrably failed, and after this check
destroyed our own first result we preferred declining.
...
% each failure is.
\section{Quantization and Silent Deletion}
\label{sec:external}
...
promoted to microseconds (Supplement~S36 quotes the call). A sample then enters the
...
a strictly positive difference is kept, and anything else is dropped. The message is counted as received a few lines earlier, but a non-positive latency is
...
Section~\ref{sec:generality} finds the same disposal, uncounted, in
...
every instrumented cell. At the timestamp resolution the printed median carries no
...
timestamp changes value. Write $\tau$ for the timestamp resolution --- the
\emph{quantum}, one millisecond here --- and $T_{\mathrm{true}}$\label{def:ttrue} for the
true delivery of a message, the interval $D$ would report if it were measured with an
arbitrarily fine timestamp. A sample survives when its delivery crosses a grid instant,
which for $T_{\mathrm{true}} < \tau$ happens with probability $T_{\mathrm{true}}/\tau$
(Supplement~S51 draws the geometry):
\begin{equation}
\label{eq:retention}
...
take only the $q{+}1$ values $0, 1/q, \ldots, 1$. A run lands on one of the two values
bracketing $T_{\mathrm{true}}/\tau$. The replicate spread follows:
\begin{equation}
\label{eq:quant}
...
\end{cases}
\end{equation}
Both branches are limits: a nominally commensurate rate is not exactly commensurate, and
the two configurations that sit on a vertex spread by roughly three tenths of the mid-cell
width, too few to fix a constant (Supplement~S23). A round send rate is the worst possible
choice, and exactness decides it: Supplement~S23 runs from the $q=1$ rate whose replicates span almost the whole range to the one sitting
$0.00014$ from a vertex and behaving as fully continuous.

\subsection{The grid as inference}
\label{sec:extinference}
A table of spreads is not a test, so we state one: the mean normalized distance from a
configuration's replicates to the nearest $q$-grid vertex, against the
within-configuration permutation null a continuum implies. Supplements~S54 and~S46 give
the construction and every configuration with $n \ge 5$. $\gridReject$ of the
$\gridPowered$ powered configurations reject the continuum null at $\alpha = 0.05$ after
Holm correction within the family of $\gridArms$. The tenth, $\gridUnresolvedRate$~msg/s,
rejects before correction ($p = \gridUnresolvedRaw$) and not after
($p = \gridUnresolvedHolm$); we report it as unresolved rather than as support. The
remaining $\gridFlat$ configurations, \gridFlatRates~msg/s, have no power by construction:
where
...
The gray bar beside each configuration is its own simulated null, \emph{uncorrected}; the
...
picture cannot draw. The $\gridFlat$ squares are the configurations where the two
hypotheses coincide and no test has power; \emph{the sign carries nothing} there.
Corrected $p$-values are in Supplement~S46.}
\label{fig:grid}
\end{figure}
...
dictates the configuration's class with nothing else changed. The prediction was registered before
...
the predicted $66.67$. Supplement~S31 draws all three configurations against their grid,
...
To show the law is not an artifact of one JVM, we reproduced the admission condition in an
...
(Supplement~S10). The grid behavior reappeared intact. At
...
either. Nor is the admission condition confined to the tool we measured: $\forkUnchanged$
of the $\forkChecked$ public forks we could read carry the expression unchanged, surveyed
\forkCheckedOn{}.

Nor is the disposal confined to this harness, and deletion is not the only way coarse
resolution wins. We audited \harnessAuditedWord{} tools at source
...
clamping negative durations to zero with no counter~\cite{kafka19888} (Supplement~S36).
...
\section{Scheduling and Acknowledgment Bias}
\label{sec:mechanism}
...
send-referenced span and \tti{}, which are causal chains, are negative
on \textbf{no event at all}. None of the $\spanNegAck$ messages arrived before it was
...
brokers: \kafka{} on $\spanKafkaNegAckPct\%$ of events, \redis{} on $\spanRedisNegAckPct\%$.
The causal chains are clean under each broker, so their per-broker columns are dashed.}
\label{tab:spans}
\centering
\footnotesize
...
\label{sec:blindspot}
The rejected result had looked correct: monotone in concurrency, significant after
correction ($p = 9.0\times10^{-11}$), and past every conventional check we knew
(Supplement~S43). The check does not catch everything. Load-induced inflation that stays
positive is physically possible, and our own second withdrawal is the example --- a
twentyfold end-to-end gap that was a per-run start-up cost read as a per-event constant,
causally consistent and wrong. Causal consistency is necessary, not sufficient.
...
second state and $G(\cdot)$ for the survival function of the residual delay in that state,
and take the consumer's timestamp to lie in the core. Write $T_{\mathrm{true}}$ for the
true delivery of Section~\ref{sec:extphase}: the quantity $D$ measures, equal to $D$ up to
the consumer's own timestamping jitter. Conditioning on it, the identity of
Equation~\ref{eq:negspan} becomes, to leading order,
\begin{equation}
...
model's content is on the delivery axis rather than the load one (Supplement~S2). The
...
\begin{table}[tb]
\caption{\textbf{The mechanism, by manipulation} ($\mechEventsPerCell$ matched events per
...
\label{sec:twostate}
No pair of configurations in Table~\ref{tab:mechanism} overlaps, and Supplement~S47 draws
...
$\untracedRate$ is the machine's (Supplement~S43.4).
...
median acknowledgment lag, $A = \ackLagMedianUs\,\mu$s, that is $\exposureErrTen\%$ at a
$10$~ms delivery and $\exposureErrHundred\%$ at $100$~ms. Read the other way, it is
$\exposureErrOne\%$ at $1$~ms, and below $\exposureCrossover$~ms the displacement exceeds
the delivery it is a correction to --- where sub-millisecond medians are routine in broker
benchmarking (Supplement~S48). $A$ is not a constant: across our conditions it spans
$\exposureLagLo$--$\exposureLagHi\,\mu$s, which puts the $10$~ms error at
$\exposureErrTen$--$\exposureErrTenHi\%$ and the crossover at
$\exposureCrossover$--$\exposureCrossoverHi$~ms; Supplement~S48 bands every row.
...
delivery it stands for, understating it $\understateFactor\times$ --- a median of
per-condition ratios, which is not what differencing two published medians gives
(Supplement~S48). The two brokers' clients timestamp differently, and over
...
acknowledgment lag recovers the delivery it failed to measure. The recovered median lands
within $\recoveryErrPass\%$ of the true one on runs the check passes, and within
$\recoveryErrFail\%$ on the runs it rejects --- the population a remedy tried only against
clean runs never meets.
...
Log--log least squares over the four payload levels of Section~\ref{sec:twostate}
returns a slope of $\tailSlope$ ($\tailSlopeCI$, Student-$t$ at two degrees of freedom);
the fit is in Supplement~S32, and four points do not earn an equation here. An earlier
version read the kernel trace as an independent confirmation of it. We withdraw the claim,
because estimating the traced index properly is what refuted it.

Two estimators of the traced tail index disagree sixfold, and a bootstrap goodness-of-fit
rejects the fitted power law ($p \tracedGofP$ over $\tracedGofBoot$ replicates drawn from
it); both are in Supplement~S32. Over the $\tracedEvents$ traced wakeups the counts do not
...
standing $\tracedModeRatio\times$ its lower neighbor. Above it the survival is light:
grouped maximum likelihood beyond $\tracedTailFrom$~ms gives $\tracedTailAlpha$
($\tracedTailCI$), a finite variance. Neither that share nor that position has been
...
(Supplement~S44). That tick is the kernel's timer period, not the timestamp resolution of
Section~\ref{sec:external}; both are $1$~ms here by coincidence of configuration. A thread
...
$\tostLevels$ levels), so it is not a purchasing argument. The one
condition that reverses the ordering is a consumption pattern rather than the broker. A
co-located testbed --- the standard evaluation environment --- hides the choice that
decides the outcome (Supplement~S48).
...
\redis{}'s after it, so the transport proxy carries one client's parse and not the other's:
$\spanHandlingWorstUs\,\mu$s, or $\spanHandlingSharePct\%$ of \redis{}'s own median
delivery. That is a fiftieth of the equivalence margin above, and it runs \emph{against}
the per-broker difference of Table~\ref{tab:spans} rather than producing it. \tti{} is free
of it, because both parses fall inside \tti{} (Supplement~S43.1).
...
negative-span rate $\rtFactorLow$--$\rtFactorHigh\times$ at unchanged background load. Two
caveats: the floor is not zero, so the check stays. And a real-time timestamping thread
changes the system it measures, so where that is unacceptable, report retention instead.
...
timestamp resolution itself. Publish beside the latency the retention rate, the timestamp
...
it~\cite{rfc4656} (Supplements~S39 and~S40 give the precedents). The sign is the channel:
...
records. The admission condition deletes the same samples and the grid law is unchanged.
...
admission condition and genuine negatives survive it (Supplements~S37 and~S43.6). A better
...
deepest negative span (Supplement~S42).
...
benchmark and reproduced in one independent harness. The \harnessSilentWord{} tools of
Section~\ref{sec:generality} are readings of source rather than measured deployments. The
benchmark's distributed mode failed on every attempt, so its cross-host exposure rests on
source reading and our own two-host harness (Supplement~S4). Exposure depends on a
...
in the output. Two timestamps written by threads that wait for a core independently invert
the delivery between them whenever those waits differ by more than it. A timestamp quantized
