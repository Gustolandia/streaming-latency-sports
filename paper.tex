...
162-1~\cite{hp1970tia} carries our retention identity, the reduced fraction our grid law
turns on, our ceiling
...
take only the $q{+}1$ values $0, 1/q, \ldots, 1$, the \emph{vertices} of its grid. A run
lands on one of the two vertices bracketing $T_{\mathrm{true}}/\tau$. The replicate spread follows:
\begin{equation}
\label{eq:quant}
...
within-configuration permutation null that a \emph{continuum} --- retention varying smoothly
with $T_{\mathrm{true}}/\tau$, as an incommensurate rate gives --- implies.
Supplements~S44 and~S23 give
