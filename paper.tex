...
running the brokers under containers, produced our withdrawn first result and enters only
through the audit, whose rejection count spans both (Section~\ref{sec:audit}).
...
earlier version read the kernel trace as confirming it. We withdraw the claim: two
estimators disagree sixfold, and a bootstrap goodness-of-fit rejects the fitted power law
($p \tracedGofP$ over $\tracedGofBoot$ replicates), in Supplement~S15.
...
than trail: they fall $\tracedTailFallA\times$ and $\tracedTailFallB\times$ over the next
\tracedTailOctaves{} octaves and then $\tracedTailFallLast\times$, where a power law falls by
one factor every time. The same bootstrap rejects a fit here too (Supplement~S15). Neither
...
and \tti{} is free of it (Supplement~S40.1).
