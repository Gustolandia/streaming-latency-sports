...
def render_registry_table():
    ...
    label = {"positive_only_filter": "filter",
    ...
    for (harness, path), v in sorted(folded.items()):
        kinds = sorted(v["kinds"] & set(label))
        disposal = ", ".join(label[k] for k in kinds) if kinds else "---"
        lines.append("%s & %s & %s & %s & %s \\\\" % (
            _tt(harness), path, v["clock"], disposal,
            "yes" if v["counts_discards"] else "no"))
    lines += ["\\bottomrule", "\\end{tabular}"]
    _ = cite  # citations stay in the prose, where a reader meets them in context
    return "\n".join(lines) + "\n"
