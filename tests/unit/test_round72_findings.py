...
class TestEveryTypedNumeralInTheMainTextIsADecision:
    r"""The sweep round 72's referee ran, kept, so the next round does not have to run it.

    Stripping every `\macro` from `paper.tex` leaves the numerals a human typed. Twenty-eight
    remained when the referee looked, and two of them -- `0.1`--`0.5` ms, printed once for the
    transport proxy and once for the delivery -- were the only measured property of this
    corpus among them. Everything else was a third party's figure being quoted, a setting the
    campaign chose, an evaluation point the prose picked, or a value that is a multiple of the
    timestamp resolution by definition.

    That distinction is the whole content of standard A2d, and it is only checkable if the
    legitimate literals are enumerated. So they are, by category and with a reason. A new
    literal fails this test until somebody adds it here, which is the point: typing a number
    into this manuscript becomes a decision that is written down rather than a habit.
    """
    ...
    ALLOWED = {
        ...
        "0": "the bottom of the audit-threshold sweep, a range we chose; its top, twenty "
             "per cent, is one token with the percent sign and does not reach this pattern",
        "0.1": "the precision at which the corpus-wide Wilson intervals are being described, "
               "not a measurement",
        ...
        "200": "the payload of the named cell, a setting",
        "500": "the send rate of the named cell, a setting",
    ...
    def test_no_typed_numeral_is_unaccounted_for(self, paper):
        import re
        prose = re.sub(r"(?m)^%[^\n]*", "", paper)
        body = re.sub(r"\\[A-Za-z@]+", " ", prose)
        found = {}
        for m in re.finditer(r"\$(-?\d[\d,.{}]*)\$", body):
            ctx = " ".join(body[max(0, m.start() - 80):m.start() + 80].split())
            found.setdefault(m.group(1), ctx)
    ...
    def test_the_inventory_has_not_gone_stale(self, paper):
        ...
        prose = re.sub(r"(?m)^%[^\n]*", "", paper)
        body = re.sub(r"\\[A-Za-z@]+", " ", prose)
        present = {m.group(1) for m in re.finditer(r"\$(-?\d[\d,.{}]*)\$", body)}
        stale = sorted(set(self.ALLOWED) - present)
...
class TestTheNewReadersFailQuietlyAndCountCorrectly:
    """Both round-72 readers are guarded, and both guards are exercised here.

    `mechanism_macros` catches OSError from each of its eight reads, which is only a promise
    if something has seen the reader raise it.
    """

    def test_an_absent_profile_ledger_is_no_corpus_rather_than_a_crash(self, tmp_path):
        import stat_intervals
        assert stat_intervals.workload_corpus(
            path=str(tmp_path / "absent.csv")) is None

    def test_an_absent_span_ledger_is_no_medians(self, tmp_path):
        import stat_intervals
        assert stat_intervals.span_medians(path=str(tmp_path / "absent.csv")) == {}

    def test_a_column_that_is_not_there_is_dropped_not_guessed(self, tmp_path, monkeypatch):
        import stat_intervals
        monkeypatch.setattr(stat_intervals, "_rows", lambda *a: [
            {"median_D_us": "700"}, {"median_D_us": "900"}])
        med = stat_intervals.span_medians()
        assert set(med) == {"D"}, "A and S have no column here and must not be invented"
        assert med["D"] == (700.0, 800.0, 900.0), "an even count takes the midpoint"

    def test_an_unreadable_cell_is_skipped_and_the_rest_still_count(self, monkeypatch):
        import stat_intervals
        monkeypatch.setattr(stat_intervals, "_rows", lambda *a: [
            {"median_D_us": "700"}, {"median_D_us": ""}, {"median_D_us": None},
            {"median_S_us": "50"}, {"median_D_us": "2950"}])
        med = stat_intervals.span_medians()
        assert med["D"] == (700.0, 1825.0, 2950.0)
        assert med["S"] == (50.0, 50.0, 50.0)
